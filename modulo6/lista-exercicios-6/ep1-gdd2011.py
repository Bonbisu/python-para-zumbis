...
def get_outras(lista_texto):
    '''
    Retorna todos os caracteres possíveis no texto
    '''
    letras = {}
    for p in txtA:
        for l in p:
    ...
    return list(letras.keys())


outras = get_outras(txtA) + get_outras(txtB)
# Preposições em txtB
print(len([p for p in txtB if p[0] in 'ftc' and p[-1] in outras]))


def get_verbos(lista_texto):
    verbos = []
    ppessoa = []
    for p in lista_texto:
        if len(p) == 5 and p[-1] in outras:
            verbos.append(p)
            if p[0] in outras:
                ppessoa.append(p)
    return verbos, ppessoa
