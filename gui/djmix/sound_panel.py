...
class SoundPanel(Frame):
    ...
    def __init__(self, app, mixer, sound_file):
        ...
        track_button = Checkbutton(
            self, variable=self.track_playing, command=self.track_toggle, text=sound_file)
        track_button.pack(side=LEFT)
        self.volume = DoubleVar()
        self.volume.set(self.track.get_volume())
        volume_scale = Scale(self, variable=self.volume, from_=0.0, to=1.0, resolution=0.01,
                             command=self.change_volume, label='Volume', orient=HORIZONTAL, length=200)
