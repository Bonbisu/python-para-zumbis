...
def inicio_fim_diferente(telefone):
    """
...
for numero in s.split():
    if not have_consecutive(numero) and soma_par(numero) and inicio_fim_diferente(numero):
        validos.append(numero)

print('%d números válidos de %d' % (len(validos), len(s.split())))
# print('%10s %10s %10s' % (numero, 'consecutivo:', have_consecutive(numero)))
# print('%10s %10s %10s' % (numero, 'soma_par:', soma_par(numero)))
# print('%10s %10s %10s' %
#       (numero, 'diferente:', inicio_fim_diferente(numero)))
